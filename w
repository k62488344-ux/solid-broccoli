...
local function startAutoPlayLeft()
    ...
    aplConn = RunService.Heartbeat:Connect(function()
        ...
        if aplPhase == 1 then
            ...
            local md = d.Unit; hm:Move(md, false)
            h.AssemblyLinearVelocity = Vector3.new(md.X * goingSpeed, h.AssemblyLinearVelocity.Y, md.Z * goingSpeed)
        elseif aplPhase == 2 then
            local d = Vector3.new(LEND.X - h.Position.X, 0, LEND.Z - h.Position.Z)
            if d.Magnitude < 1 then
                aplPhase = 0; hm:Move(Vector3.zero, false)
        ...
        elseif aplPhase == 4 then
            ...
            if d.Magnitude < 1 then
                hm:Move(Vector3.zero, false); h.AssemblyLinearVelocity = Vector3.new(0, 0, 0)
...
local function startAutoPlayRight()
    ...
    aprConn = RunService.Heartbeat:Connect(function()
        ...
        elseif aprPhase == 2 then
            ...
            if d.Magnitude < 1 then
                aprPhase = 0; hm:Move(Vector3.zero, false)
                h.AssemblyLinearVelocity = Vector3.new(0, 0, 0)
...
UserInputService.InputBegan:Connect(function(input, gpe)
    ...
    if input.KeyCode == Enum.KeyCode.J then
        if aplOn then stopAutoPlayLeft() else startAutoPlayLeft() end
...
lp.CharacterAdded:Connect(function()
    if aplOn then task.wait(1); startAutoPlayLeft()
    elseif aprOn then task.wait(1); startAutoPlayRight() end
end)
...
do
    ...
    function startBatAimbot()
        ...
        if aplOn then stopAutoPlayLeft(); print("Auto Play Left disabled by Bat Aimbot") end
        if aprOn then stopAutoPlayRight(); print("Auto Play Right disabled by Bat Aimbot") end
...
function createAutoPlayGui()
    ...
    local function updateButtons()
        leftBtn.BackgroundColor3  = aplOn and Color3.fromRGB(0,200,80) or Color3.fromRGB(25,25,25)
        leftBtn.Text              = aplOn and "L ✓" or "LEFT"
        rightBtn.BackgroundColor3 = aprOn and Color3.fromRGB(0,200,80) or Color3.fromRGB(25,25,25)
        rightBtn.Text             = aprOn and "R ✓" or "RIGHT"
    end
        ...
        if scActive then
            ...
            if not speedConnection then
                speedConnection = RunService.Heartbeat:Connect(function()
                    ...
                    goingSpeed  = tonumber(speedBox.Text) or 59
                    returnSpeed = tonumber(stealBox.Text) or 30
                    if aplOn or aprOn then return end
